...
ここで、ベクトル $\boldsymbol{\phi}\bar{\textbf{w}}$ は無次元のエネルギー、力、応力テンソルを含む。また、共分散行列 $\boldsymbol{\sigma}$ は各無次元成分の分散を与える。共分散行列の2項目は回帰係数 $\textbf{w}$ における不確実性に対応しており、学習データが必要な構造どうかの判断が可能である。
## ベイズ線形回帰
