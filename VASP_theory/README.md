...
であるから、可換であるとわかる。さらに、ハミルトニアン演算子と交換演算子が同時固有状態を持つことがわかる。また、2回の交換で元に戻るということは、固有値 $\lambda$ は、  
...
で求められる。  
　実際に、この方程式を解く際は、__SCF法__ が用いられるが、詳細は後の章で述べる。ちなみに、このSCF法についても、人の手で解くにはあまりにも複雑すぎるため、コンピュータの登場まで用いられることはなかったそうである。SCF法の流れとしては、  

   1. 分子の情報と初期参照分子軌道 $\{ \phi_i^0 \}$ を設定
   2. 参照分子軌道でFock演算子 $\hat{F}$ の2電子積分を計算
   3. 計算したFock演算子でHartree-Fock方程式を解く
   4. 得られた分子軌道を参照分子軌道を比較し、自分で設定した閾値以下の差ならその得られた分子軌道を解とする。閾値以上の差なら、その得られた分子軌道を参照分子軌道とし、2に戻る

という流れである。VASPではINCARタグのNELMがこのSCFを繰り返す回数を定めている。

### 1.1.5. Roothaan法
&emsp;コンピュータの登場に伴い、Roothaanは1951年にHartree-Fock方程式をコンピュータで解くための方法であるRoothaan法を提案した。同じ年に、Hallも同じ方法を提案した。注目すべき点は、計算速度を向上させるため、1950年にBoysが提案した[Gauss型基底関数](#08-ガウス型関数)を用いていることである。  
